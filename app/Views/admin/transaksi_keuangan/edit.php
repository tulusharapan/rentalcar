<?= view('admin/layouts/header', ['title' => 'Edit Transaksi Keuangan - Aplikasi']) ?>
...
<?= view('admin/layouts/topbar', [
    'pageTitle'    => 'Edit Transaksi Keuangan',
    'pageSubtitle' => 'Perbarui data pemasukan atau pengeluaran.',
...
<section class="content-area">
    ...
    <div class="panel p-4">
        <form action="<?= site_url('admin/transaksi-keuangan/update/' . $transaksi['id']) ?>" method="post">
            <?= csrf_field() ?>
                ...
                <div class="col-12 col-lg-3">
                    ...
                    <select class="form-select" id="jenis" name="jenis" required>
                        <?php foreach ($jenisOptions as $jenis) : ?>
                            <option value="<?= esc($jenis) ?>" <?= old('jenis', $transaksi['jenis']) === $jenis ? 'selected' : '' ?>><?= esc($jenis) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-12 col-lg-3">
                    ...
                    <select class="form-select" id="kategori_keuangan_id" name="kategori_keuangan_id" required>
                        ...
                        <?php foreach ($kategoriOptions as $kategori) : ?>
                            <option value="<?= esc($kategori['id']) ?>" <?= old('kategori_keuangan_id', $transaksi['kategori_keuangan_id']) == $kategori['id'] ? 'selected' : '' ?>><?= esc($kategori['kategori']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-12 col-lg-3">
                    <label for="nominal" class="form-label fw-semibold">Nominal</label>
                    <input type="text" class="form-control currency-input" id="nominal" name="nominal" value="<?= old('nominal', $transaksi['nominal']) ?>" required>
                </div>
                <div class="col-12">
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                ...
                <button type="submit" class="btn btn-dark">Update Transaksi</button>
            </div>
        </form>
    </div>
</section>
